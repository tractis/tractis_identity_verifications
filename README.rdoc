...
In order to create certified profiles [\*you will need to specify that the verification will be public*]:

  <%= identity_verification_gateway("Identify!", "API-KEY", {:action => 'create'}, [\*{:public_verification => true}*]) %>

When you create a public verification you will receive an extra parameter from the gateway called *verification_url*.
